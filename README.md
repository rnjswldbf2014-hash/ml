...
OpenCL 백엔드가 들어있긴 한데 **기본으로 꺼둡니다.** 테스트한 기계
(AMD gfx1035 내장 GPU)에서 CPU 배치 경로보다 **17~25배 느리기 때문입니다.**
...
커널이 타일링도 메모리 병합 접근도 없는 순진한 행렬곱이라 이론 성능의 0.2% 정도밖에
못 냅니다. 결과 자체는 맞아서(오차 1e-5 수준) 느려진 걸 알아채기도 어렵습니다.

굳이 켜보려면 `MYML_GPU=1`, 지금 쓰고 있는지 확인하려면 `gpu_info()` 를 부르세요.
빨라진다는 걸 벤치마크로 보일 수 있게 되면 기본을 다시 켜겠습니다.
