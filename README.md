...
OpenCL 백엔드가 들어있습니다. 다만 **아주 큰 묶음에서만 약간 빨라서** 기본은 꺼둡니다.
...
| `[1024,2048,2048]` | 512 | 307.2 ms | **286.4 ms** | GPU 1.07배 |

이 정도 차이로 남의 기계에서 기본으로 켜기는 이릅니다. 켜려면 `MYML_GPU=1`(강제)
또는 `MYML_GPU=auto`(문턱값), 지금 쓰고 있는지 확인하려면 `gpu_info()` 를 부르세요
(`runs` 가 0이면 안 쓰고 있는 것입니다).

내장 GPU 는 CPU 와 같은 메모리를 나눠 씁니다. 외장 GPU 라면 훨씬 유리할 텐데
확인할 방법이 없어서 측정한 것만 적어둡니다.
