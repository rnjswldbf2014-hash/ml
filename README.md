...
입력이 번호(글자, 단어 같은 것)라면 `tok` 을 씁니다.
번호만 넘기고, 벡터로 펼치는 일은 안에서 합니다.

```python
from my_ml import make, tok, attn

글자 = sorted(set(텍스트))
번호 = {c: i for i, c in enumerate(글자)}

# 앞 16글자를 보고 다음 글자 맞히기
ai = make("내모델", [tok(len(글자), 16, 32), 256, attn(8), 256], 글자)

ai.sl([번호[c] for c in "안녕하세요 반갑습니"], "다")
```

`tok(어휘수, 길이, 폭)` — 번호 `길이`개를 받아 각각 `폭`칸으로 펼칩니다.
펼치는 표도 같이 학습됩니다.

one-hot 을 파이썬에서 만들어 넘기면 데이터가 수백 배로 불어납니다. `tok` 을 쓰면
번호만 넘어갑니다.
