...
토큰 열을 이어 쓰는 모델입니다. 정답을 따로 줄 필요가 없습니다 — 다음 토큰이 곧 정답입니다.

**토큰은 0 이상 vocab 미만의 정수입니다.** 글자나 단어를 번호로 바꾸는 건 쓰는 쪽에서 합니다.
...
# 글자 <-> 번호 (직접)
글자 = sorted(set(텍스트))
번호 = {c: i for i, c in enumerate(글자)}

lm = make_lm("내모델", vocab=len(글자), dim=64, layers=2, heads=4, ctx=32)

lm.sl([번호[c] for c in 텍스트], steps=1000)      # 학습

나온것 = lm.gen([번호["안"], 번호["녕"]], 30)     # 이어 쓰기 (숫자 리스트)
print("".join(글자[i] for i in 나온것))
lm.save()
```
...
```python
lm.gen(시작토큰, 40, temp=0.5, topk=5)
lm.loss(토큰들)                  # 학습 없이 얼마나 잘 맞히는지
```
