...
- `tests/` — `regression.py` (sl 경로 결정성), `rl.py` (RL 묶음 경로 동치 +
  ...
  `gpu.py` (기본이 GPU 를 안 쓰는지 + 강제하면 결과가 맞는지)
...
GPU 는 큰 묶음을 연달아 학습할 때만 이득이다. 기본은 끔 (AMD gfx1035 내장 실측):
...
**기본을 켜기 전에 반드시 같이 볼 것** — 위는 "학습만 연달아" 의 숫자다:

| 쓰는 방식 | `[512,1024,1024]` x1024 | `[128,256,256]` x256 |
|---|---|---|
| 학습만 연달아 | GPU 2.16배 | CPU 1.05배 |
| 학습 ↔ 평가 번갈아 | GPU 1.66배 | CPU 1.57배 |
| 한 번만 학습 | CPU 25배 | CPU 209배 |

- 첫 호출에 **약 1.3초** (OpenCL 이 런타임에 커널을 컴파일한다. 망 크기와 무관).
  `clGetProgramInfo(CL_PROGRAM_BINARIES)` + `clCreateProgramWithBinary` 로 디스크에
  캐시하면 없앨 수 있다 — 아직 안 했다.
- `predict()` 가 끼면 가중치가 내려갔다 다시 올라간다. `_gpuSyncBack()` 이 w, b, m, v 를
  통째로 내리는데 순전파는 w, b 만 필요하다 — 나눠 내리면 이 패턴이 개선된다.
  아직 안 했다.

**커널** — 32x32 타일 + 로컬 메모리 + 레지스터 블로킹(스레드 하나가 WPT=4개 출력).
고칠 때 깨뜨리기 쉬운 것:
- 전역 읽기는 `get_local_id(0)` 이 "붙어있는" 축을 훑어야 한다. 맨 처음 커널이 이걸
  어겨서 이론 성능의 0.2% 밖에 못 냈다 (CPU 보다 17~25배 느렸다). `dZ` 는
  `[count][outSz]` 라 j 가, `xs`/`wt` 는 k 가 붙어있어서 **커널마다 어느 인덱스를
  lane 에 태우는지가 다르다.**
- 로컬 배열은 `[TS][TS+1]` 패딩. 빼면 뱅크 충돌.
- 누적 루프의 로컬 읽기는 하나가 broadcast, 하나가 연속이어야 한다.
- `gpu_cl.d` 커널 소스의 `TS`/`WPT` 와 D 쪽 `TS`/`WPT`/`RTS` 는 반드시 같아야 한다
  — 어긋나면 오류도 없이 조용히 틀린 값이 나온다.

**가중치 상주** — w, b, m, v 를 GPU 에 남겨둔다. 연달아 학습할 때 이 전송이 호출당
고정비의 대부분이었다. 호스트가 만지려 할 때만 `_gpuSyncBack()` 으로 회수한다.

회수 시점은 **`Network.gpuSync` 훅**이 잡는다. `forward`/`forwardBatch`/`backward`/
`backwardBatch`/`step` 입구에 `_sync()` 가 박혀 있고 BlackBoxAI 가 생성자에서 꽂는다.
호출처마다 "여기서 회수" 를 적어두면 하나만 빠뜨려도 **조용히 틀린 답**이 나오므로
그렇게 하지 않았다 — 새 경로를 추가해도 저 다섯 중 하나는 반드시 지나간다.
신경망을 안 거치고 필드를 직접 훑는 곳(`save()`, `export_weights`)만 따로 부른다.
**새 코드가 `net.lins[].w` 류를 직접 읽는다면 `syncFromGpu()` 를 먼저 불러야 한다.**

`_gpuSlMany` 가 중간에 실패하면 `_gpuResident` 를 안 세우므로 호스트 값은 그대로고
(GPU 버퍼의 부분 갱신은 다음 업로드가 덮는다), `t` 도 전부 성공한 뒤에 올린다.

`tests/gpu.py` 가 지킨다: 기본이 GPU 를 안 쓰는지, 타일 배수가 아닌 크기들, 그리고
GPU/CPU 를 번갈아 학습시킨 뒤 **저장된 가중치와 `predict()` 둘 다** 확인한다
(`predict()` 는 `save()` 전에 찍는다 — save 가 스스로 동기화해서 가려버리므로).
세 동기화 지점(forward 훅 / step 훅 / save)을 각각 다른 시퀀스가 잡는다 —
하나씩 지워보고 확인했다.

경로 자체는 순수 Linear 망 + 헤드 1개 + `cos` 출력만 지원한다 (attn/each, 다중
헤드, pick 헤드는 CPU 배치 경로로 자동 폴백). NVIDIA/AMD 둘 다 OpenCL 하나로 커버
— CUDA 전용 경로는 없음.

`gpu_info()` 로 GPU 를 실제로 쓰고 있는지 확인한다 (`runs` 가 0이면 안 쓴 것).
