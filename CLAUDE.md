...
| 층 | 묶음 | CPU | GPU | |
|---|---|---|---|---|
| `[128,256,256]` | 64 | 1.5 ms | 2.7 ms | CPU 1.8배 |
| `[512,1024,1024]` | 1024 | 174.2 ms | 87.6 ms | GPU 1.99배 |
| `[1024,2048,2048]` | 512 | 335.8 ms | 137.5 ms | GPU 2.44배 |
...
**옵티마이저 상태 상주** — Adam 의 m, v 는 GPU 에 남겨둔다 (호출당 고정비의 대부분이
이 전송이었다). 호스트가 읽기 전에 `_gpuSyncOpt()` 로 내린다. 동기화 지점은 둘뿐:
`_stepCPU()` 와 `save()`. CPU 쪽 `net.step()` 은 **반드시** `_stepCPU()` 를 거쳐야 한다
(Jepa 는 직접 부르므로 거기서 따로 `_gpuSyncOpt()` 한다).
가중치(w, b)는 상주시키지 않는다 — 호스트가 순전파에서 바로 읽어서, 빠뜨리면
"관성이 옛날 값" 이 아니라 "예측이 틀림" 이 된다.

`tests/gpu.py` 가 지킨다: 기본이 GPU 를 안 쓰는지, 타일 배수가 아닌 크기들, 그리고
GPU/CPU 를 번갈아 학습시켜 옵티마이저 상태가 살아남는지 (두 동기화 지점을 각각 다른
시퀀스가 잡는다 — 하나를 지워보고 확인했다).
