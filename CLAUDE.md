...
GPU 는 아주 큰 묶음에서만 약간 빠르다. 그래서 기본은 끔이다 (AMD gfx1035 내장 실측):

| 층 | 묶음 | CPU | GPU | |
|---|---|---|---|---|
| `[128,256,256]` | 64 | 1.5 ms | 5.7 ms | CPU 3.7배 |
| `[128,256,256]` | 1024 | 21.0 ms | 21.8 ms | 대등 |
| `[512,1024,1024]` | 1024 | 174.5 ms | 136.2 ms | GPU 1.28배 |

커널은 16x16 타일링 + 로컬 메모리다. 고칠 때 두 가지를 깨뜨리기 쉽다:
① 전역 읽기는 `get_local_id(0)` 이 "붙어있는" 축을 훑어야 한다 (예전 커널이 이걸
어겨서 이론 성능의 0.2% 밖에 못 냈고, 그때는 CPU 보다 17~25배 느렸다).
② 로컬 배열은 `[TS][TS+1]` 패딩. 빼면 뱅크 충돌.
`gpu_cl.d` 의 `TS` 와 `runTiled` 의 `TS` 는 반드시 같아야 한다 — 어긋나면 조용히
틀린 값이 나온다.

예전엔 기본이 `auto` 여서 OpenCL 깔린 기계에서 묶음만 커지면 사용자가 영문도
모르고 20배 느려졌다. 지금은 대등~약간 우세지만, 그 정도로 남의 기계에서 기본을
켜기는 이르다. `tests/gpu.py` 가 이걸 지킨다 (타일 배수가 아닌 크기들도 같이 검사).
