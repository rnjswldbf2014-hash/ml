...
**GPU 는 지금 켜면 안 된다.** 이 기계(AMD gfx1035 내장)에서 CPU 배치 경로보다
**17~25배 느리다.** 결과는 맞아서 알아채기도 어렵다. 원인은 커널 — 타일링도
병합 접근도 없는 순진한 행렬곱이라 이론 성능의 0.2% 밖에 못 낸다. 측정치:

| 층 | 묶음 | CPU | GPU |
|---|---|---|---|
| `[128,256,256]` | 1024 | 19.6 ms | 324 ms |
| `[512,1024,1024]` | 1024 | 183 ms | 3,240 ms |

예전엔 기본이 `auto` 여서 OpenCL 깔린 기계에서 묶음만 커지면 사용자가 영문도
모르고 20배 느려졌다. 그래서 기본을 끔으로 바꿨다. `tests/gpu.py` 가 이걸 지킨다
— 어디서든 빨라진다는 걸 벤치마크로 보이기 전엔 기본을 다시 켜지 말 것.
